...
fetch(BASE_ENDPOINT_URL + 'admin/poolparty/account', {
  ...
  .then(async (response) => {
    const accounts = (await response.json()).sort(
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      for (key in a) {
        ...
        if (key == 'lastActivity')
          a[key] = a[key] ? new Date(a[key]).toLocaleDateString() : ''
        td.innerText = a[key] || ''
        tr.append(td)
      }
  ...
  for (let i = 0; i < registration.length; i++) {
    ...
    for (key in a) {
      if (key == 'music') {
        ...
        if (key == 'people') sum += a[key]
        const td = document.createElement('td')
        if (key == 'lastActivity')
          a[key] = a[key] = a[key] ? new Date(a[key]).toLocaleDateString() : ''
        td.innerText = a[key] || ''
        tr.append(td)
      }
    }
    ...
    document.querySelector(
      '#content2 h3'
    ).innerText = `Anmeldungen (${sum} Personen)`
    ...
    removeButton.onclick = () => {
      ...
      removeButton.onclick = async () => {
        const response = await fetch(
          BASE_ENDPOINT_URL + 'admin/poolparty/item/' + a.id,
          {
            method: 'DELETE',
            headers: new Headers({
              Authorization: token,
