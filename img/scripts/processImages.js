...
;(async () => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    for (let l = 0; l < sizes.length; l++) {
      ...
      else if (imgWidth < 600 && sizeName == 'medium' && imgWidth > 400)
        width = imgWidth
      else if (imgWidth < 800 && sizeName == 'large' && imgWidth > 600)
        width = imgWidth
      ...
      data = await sharp(file.data).resize({width}).toBuffer()
